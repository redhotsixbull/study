...
exports.findConnectedComponentsIterative = findConnectedComponentsIterative;
exports.countConnectedComponents = countConnectedComponents;
exports.findConnectedComponentsUF = findConnectedComponentsUF;
...
// 반복적 DFS 버전 (스택 사용)
function findConnectedComponentsIterative(graph) {
    const visited = new Set();
    const components = [];
    for (const node in graph) {
        if (!visited.has(node)) {
...
function dfsVisitOnly(graph, node, visited) {
    visited.add(node);
    for (const neighbor of graph[node] || []) {
        if (!visited.has(neighbor)) {
...
function findConnectedComponentsUF(graph) {
...
//# sourceMappingURL=02-dfs-connected-components-solution.js.map